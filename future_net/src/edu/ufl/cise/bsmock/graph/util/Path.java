...
public class Path implements Cloneable, Comparable<Path> {
    ...
    public Set<Integer> nodes2 = new HashSet<Integer>();
    ...
    public void addFirst(Edge edge) {
        edges.addFirst(edge);
        nodes.add(Integer.valueOf(edge.getFromNode()));
        nodes2.add(Integer.valueOf(edge.getFromNode()));
        totalCost += edge.getWeight();
    }
    ...
    public boolean containAllNodes(List<Integer> list){
        return nodes.containsAll(list);
    }
